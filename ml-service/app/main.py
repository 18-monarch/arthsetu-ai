...
def root():
 return {
  ...
  "message": "ArthSetu ML Engine is running.",
...
def full_assessment(request:FullAssessmentRequest,x_api_key:str|None=Header(default=None)):
 ...
 if request.features is not None and request.profile is not None:
  profile_id=request.profile_id or "questionnaire-user"
  score=score_features(request.features.model_dump(),profile_id)
  profile_result=ProfileSummary(
   ...
   monthly_surplus=request.profile.monthly_surplus,
   emergency_fund_months=request.profile.emergency_fund_months,
   income_stability=request.profile.income_stability,
   is_demo=False,
   consent_sources=request.profile.consent_sources,
   score=score.score,
  ...
  score=score_features(p.features,p.id)
  profile_result=profile_summary(p,score)

 risk=assess_risk(request.risk_profile)
 rec=recommend(risk,request.monthly_amount,request.years)
 sim=simulate(rec.plan,request.monthly_amount,request.years)
